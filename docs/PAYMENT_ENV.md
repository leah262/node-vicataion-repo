...
## פרונטאנד (אם קיים ריפו/אפליקציה נפרדת)
...
| `VITE_API_BASE` | אופציונלי. ברירת מחדל יחסית לפרויקט הפרונטאנד (למשל `/api` עם פרוקסי לשרת). אם מצביעים לשרת מרוחק — שם חייבים להיות מוגדרים גם PayPal ו־PayMe. |
...
| משתנה | למה |
|--------|-----|
| `APP_URL` | כתובת האתר הציבורי (SPA) — קישורים במיילים. אם ריק: קישורים משתמשים בבסיס כתובת ה-API. |
| `PUBLIC_API_URL` | בסיס ה-API הציבורי (מסתיים ב־`/api`) — קישורי אישור מייל וכו׳. מומלץ בפרודקשן. |
...
2. **PayPal** — אותו Client ID בפרונטאנד (משתנה `VITE_*` לפי הפרויקט) וב־`server/.env`; אחרי שינוי `.env` — **הפעלה מחדש** של שרת הפיתוח של הפרונטאנד ושל Node.
